...
import { DiscordLogo, FacebookLogo, InstagramLogo, TwitterLogo, EnvelopeSimple } from "../twitch-scraper/social-icons"
...
export default function YouTubeDataTable({ data, subscribed }: YouTubeDataTableProps) {
            ...
            {data.length === 0 ? (
              ...
              data.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell className="text-right">{row.subscribers.toLocaleString()}</TableCell>
                  <TableCell className="text-right">{row.averageViews.toLocaleString()}</TableCell>
                  <TableCell>
