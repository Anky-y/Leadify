...
export function DashboardHeader({
  ...
  React.useEffect(() => {
    const fetchNotifications = async () => {
      ...
      try {
        ...
        if (!result.success) {
          console.error(
            `Backend error fetching notifications${result.message}`
          );
          return;
        }
  ...
  const deleteNotification = async (notificationId: number) => {
    ...
    try {
      console.log("Hi");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}notifications/delete`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
              ...
              <div className="max-h-80 overflow-y-auto">
                {isLoadingNotifications ? (
                  ...
                  notifications.map((notification) => {
                    ...
                    const descriptionNeedsExpansion = isDescriptionLong(
                      notification.description
                    );
                    const needsExpansion =
                      titleNeedsExpansion || descriptionNeedsExpansion;
                        ...
                        <div className="flex items-start gap-3 pr-8">
                          ...
                          <div className="flex-1 space-y-2 min-w-0">
                            ...
                            <div className="flex items-start justify-between gap-2">
                              <p
                                className={`text-sm font-medium leading-tight ${
                                  isExpanded || !titleNeedsExpansion
                                    ? ""
                                    : "line-clamp-1"
                                } ${
                                  notification.unread
                              ...
                              <div className="flex items-center gap-1 flex-shrink-0">
                                {badgeConfig && (
                                  <>
                                    {badgeConfig.icon && (
                                      <badgeConfig.icon
                                        className={`h-3 w-3 ${badgeConfig.iconClass}`}
                                      />
                                    )}
                                    <Badge
                                      className={`text-xs px-1.5 py-0.5 ${badgeConfig.badgeClass}`}
                                    >
                            ...
                            <div className="text-sm text-muted-foreground/90 leading-relaxed">
                              <p
                                className={
                                  isExpanded || !descriptionNeedsExpansion
                                    ? ""
                                    : "line-clamp-2"
                                }
                              >
          ...
          <DropdownMenu>
            ...
            >
              ...
              <DropdownMenuItem asChild className="cursor-pointer">
                <SafeLink isSearching={isSearching} href="/dashboard/profile">
                  <UserIcon className="mr-2 h-4 w-4" />
                  Profile
                </SafeLink>
              </DropdownMenuItem>
              <DropdownMenuItem asChild className="cursor-pointer">
                <SafeLink isSearching={isSearching} href="/dashboard/settings">
                  <Settings className="mr-2 h-4 w-4" />
                  Settings
                </SafeLink>
              </DropdownMenuItem>
              <DropdownMenuItem asChild className="cursor-pointer">
                <SafeLink isSearching={isSearching} href="/dashboard/billing">
